    ...
    for food in visible_food:
        ...
        for cluster in clusters:
            if _norm(food_pos - cluster["pos"]) < merge_distance:
                cluster["pos"] = (cluster["pos"] + food_pos) / 2.0
                cluster["value"] += food_value
                break
        else:
            clusters.append({"pos": food_pos, "value": food_value})
...
def _best_split_target(
    ...
    split_reach = 3.0 * child_radius + SPLIT_EJECT_SPEED * SPLIT_REACH_EXTRA_TICKS
...
def _blocked_angles(ctx: _Ctx) -> tuple[list[tuple[float, float]], list[np.ndarray], bool]:
    ...
    # Early game: lower threat ratio so fewer directions are blocked → more aggressive
    ...
    for blob in ctx.visible_blobs:
        ...
        blob_pos = _pos(blob)
        blob_radius = _radius(blob)
        edge_distance = _edge_distance(ctx.player_pos, ctx.player_radius, blob_pos, blob_radius)

        if blob_radius <= ctx.player_radius * threat_ratio:
            continue
        ...
        if edge_distance <= detection_distance:
            interval = _danger_cone(ctx.player_pos, blob_pos, reach)
            if interval is not None:
                blocked.append(interval)
                has_threat = True
            emergency_away.append(-_unit(blob_pos - ctx.player_pos) * (reach / max(1.0, edge_distance)))
    ...
    for virus in ctx.visible_viruses:
        ...
        if dangerous and (close_enough_to_clip or close_enough_to_matter):
            reach = virus_radius + ctx.player_radius + SAFETY_MARGIN
...
def _best_goal_angle(ctx: _Ctx, previous_angle: float) -> tuple[float, bool]:
    ...
    for cluster in ctx.food_clusters:
        ...
        score = cluster["value"] * FOOD_VALUE_WEIGHT - distance + heading_stickiness
        if score > best_score:
            best_score = score
            best_angle = angle
