...
# Turned off for timeout safety
MAX_BLOBS_FOR_PREDICTION = 0
...
def enemy_score(cache, player, future_x, future_y, danger_weight, hunt_weight, hunt_ratio, merged_safe_ratio):
    ...
    future_dangerous = (
        (potential_rads > player_radius * 1.15) &
        (edge_dists < player_radius * 5.0)
    )
...
def split_path_safe(cache, player_pos, split_radius, split_landing):
    virus_locs = cache["virus_locs"]
    virus_rads = cache["virus_rads"]
...
def get_split_decision(game, move_direction, cache):
    ...
    split_radius = player.radius / np.sqrt(2.0)
    ...
    edge_dists = dists - player.radius - blob_rads
    ...
    for idx in candidate_indices:
        ...
        if not split_path_safe(cache, player_pos, split_radius, split_landing):
            continue
    ...
    if blobs:
        ...
        if len(blob_rads) <= MAX_BLOBS_FOR_PREDICTION:
            for i in range(len(blob_rads)):
                ...
                can_eat = predator_rad > blob_rads * EAT_RATIO
...
def enemy_escape_direction(cache, player, step_distance):
    ...
    for direction in DIRECTIONS:
        ...
        if len(cache["own_locs"]) > 1:
            score += multi_blob_safety_score(cache, player, future_x, future_y)
...
def override_direction(cache, player, step_distance):
    ...
    if kill_dir is not None:
        ...
        if len(cache["own_locs"]) > 1:
            safe_kill = (
                safe_kill and
                multi_blob_safety_score(cache, player, future_x, future_y) > -OFF_MAP_PENALTY / 2
            )
        ...
        for current_score, x, y, first_direction, previous_direction in beam:
            for direction in DIRECTIONS:
                ...
                if step == 1:
                    ...
                    if len(cache["own_locs"]) > 1:
                        position_score += multi_blob_safety_score(
                            cache,
                            player,
                            future_x,
                            future_y,
